...
pub struct MessageTimings {
    pub token: String,
    pub ts_meausred_before_send: u64,
    pub ts_measured_after_send: u64,
    pub ts_measured_received: u64,
    pub ts_server: u64,
}

//token:hex,
//ts_meausred_before_send:ts,
//ts_measured_after_send:ts,
//ts_measured_received:ts,
//ts_server:ts

pub static global_message_map: LazyLock<stdMutex<HashMap<String, Vec<MessageTimings>>>> = LazyLock::new(|| {
...
impl<T: Resource> Drop for ResourceManager<T> {
    fn drop(&mut self) {
        let my_ref = self
            .death_signal
            .take()
            .expect("Death empty; already dropped?");
        tokio::spawn(async move { my_ref.send(()).await.unwrap() });
    }
}
...
impl<T: Resource + 'static> ResourceManager<T> {
    pub fn new<B: BackoffBuilder + 'static>(
        ...
        let (retry_send, mut retry_recv) =
            mpsc::channel::<oneshot::Sender<Result<(), ResourceFailure>>>(99999);
        ...
        tokio::spawn(async move {
            ...
            'stop: loop {
                ...
                loop_manager
                    .resource_state
                    .send_replace(ResourceState::Generating);
                debug!("Resource {}: generating", loop_manager.name);
                let mut result = tokio::time::timeout(
                    generate_timeout,
                    loop_manager.resource.generate_unwind_safe(),
                )
                .await
                .map_err(|e| PushError::ResourceGenTimeout(e))
                .and_then(|e| e);
                ...
                while let Err(e) = result {
                    ...
                    loop_manager
                        .resource_state
                        .send_replace(ResourceState::Failed(failure));
                    if is_final {
                ...
                *loop_manager.refreshed_at.lock().await = SystemTime::now();
                ...
                loop_manager
                    .resource_state
                    .send_replace(ResourceState::Generated);
    ...
    async fn refresh_option(&self, now: bool) -> Result<(), PushError> {
        ...
        Ok(tokio::time::timeout(MAX_RESOURCE_WAIT, confirm)
            .await
...
impl Default for KeyedArchive {
    fn default() -> Self {
        KeyedArchive {
            ...
            top: KeyedArchiveTop { root: Uid::new(0) },
...
impl KeyedArchive {
    ...
    pub fn archive_item(item: Value) -> Result<Value, PushError> {
        if let Ok(archive) = plist_to_string(&item) {
