...
use crate::{
    ...
    error::PushError,
    mmcsp::{
        self, authorize_get_response, authorize_put::put_data::Chunk,
        authorize_put_response::UploadTarget, Container as ProtoContainer, HttpRequest,
    },
    ...
    let get = mmcsp::AuthorizePut {
        data: inputs
            ...
            .map(|(prepared, object, _)| mmcsp::authorize_put::PutData {
                ...
                token: Some(object.clone().unwrap_or_default()), 
                chunks: prepared
                    .chunk_sigs
                    .iter()
        ...
        .map(|(prepared, _, container)| {
            ChunkedContainer::new(
                prepared
                    ...
                    .collect(),
                container.take().expect("Duplicate PUT containers??"),
            )
        })
        .collect::<Vec<_>>();

    let index: HashMap<String, ChunkDesc> = inputs
        .iter()
...
fn log_edge_headers(headers: &HeaderMap) {
    // ---- via ----
    if let Some(via) = headers.get("via") {
        match via.to_str() {
            Ok(v) => log::error!("via: {}", v),
            Err(e) => log::error!("via header invalid UTF-8: {}", e),
        }
    } else {
        log::error!("via header not present");
    }

    // ---- x-apple-edge-info ----
    if let Some(edge_info) = headers.get("x-apple-edge-info") {
        match edge_info.to_str() {
            Ok(encoded) => {
                log::error!("x-apple-edge-info (base64): {}", encoded);

                match general_purpose::STANDARD.decode(encoded) {
                    Ok(bytes) => {
                        //log::error!(
                        //    "x-apple-edge-info (raw bytes): {:?}",
                        //    bytes
                        //);

                        log::error!(
                            "x-apple-edge-info (lossy utf8): {}",
                            String::from_utf8_lossy(&bytes)
                        );
                    }
                    Err(e) => {
                        log::error!(
                            "failed to base64-decode x-apple-edge-info: {}",
                            e
                        );
                    }
                }
            ...
            Err(e) => {
                log::error!(
                    "x-apple-edge-info header invalid UTF-8: {}",
                    e
                );
            }
        }
    } else {
        log::error!("x-apple-edge-info header not present");
    }
}
...
impl MMCSGetContainer {
    ...
    async fn ensure_stream(&mut self) -> Result<(), PushError> {
        if self.response.is_none() {
            ...
            log_edge_headers(response.headers());
...
pub async fn authorize_get(
    config: &MMCSConfig,
    url: &str,
    files: &[(Vec<u8>, &str, impl WriteContainer + Send + Sync)],
) -> Result<AuthorizedOperation, PushError> {
    let confirmation = mmcsp::AuthorizeGet {
        item: files
            .iter()
            .map(|(sig, object, _)| mmcsp::authorize_get::Item {
...
pub async fn get_mmcs(
    ...
    files: Vec<(Vec<u8>, &str, impl WriteContainer + Send + Sync)>,
    progress: impl FnMut(usize, usize) + Send + Sync,
) -> Result<(), PushError> {
    let mut files = files
        .into_iter()
    ...
    log::error!("url {} body {:x?} dsid {}", url, body, dsid);
