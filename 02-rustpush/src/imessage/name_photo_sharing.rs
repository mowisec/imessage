...
use crate::{
    cloudkit::CloudKitUploadRequest,
    mmcsp::container,
...
use crate::{
    cloudkit::{
        prepare_cloudkit_put, record_identifier_from_string, CloudKitClient, CloudKitContainer,
        CloudKitOpenContainer, CloudKitOperation, CloudKitSession, QueryResult,
    },
...
impl<P: AnisetteProvider> ProfilesClient<P> {
    ...
    pub async fn get_record(
        ...
        let records = container
            .get_record(
                &session,
                &self.client,
                cloudkit_proto::AssetsToDownload {
        ...
        let raw_poster = if message.poster.is_some() {
            Some(records.get_record::<IMessageRawPosterRecord>(&poster_id))
        } else {
            None
        };
        ...
        if !assets.is_empty() {
            container
                .get_assets(&self.client, &records.assets, assets)
                .await?;
        }
    ...
    pub async fn set_record(
        ...
        let mut asset = container
            .upload_asset(&session, &self.client, upload_requests)
            .await?;

        let mut raw_ops = vec![container.save_records(
            &record_id,
        ...
        if let Some(poster) = &record.poster {
            raw_ops.push(container.save_records(
                &poster_id,
        ...
        if let Err(e) = container
            .preform_operation(&session, &self.client, &raw_ops)
            .await
        {
    ...
    async fn delete_my_record(&self, record_id: &str) -> Result<(), PushError> {
        ...
        self.get_container()
            .await?
            .delete_record(&session, &self.client, &record_id)
            .await?;
        Ok(())
    }
    ...
    > {
        ...
        let (mut results, assets) = container
            .query_record::<_, IMessageRawNicknameRecord>(
                &session,
                &self.client,
                CloudKitOperation::QueryRecord {
                    ...
                    filters: vec![cloudkit_proto::query::Filter {
                        ...
                        field_value: Some(
                            cloudkit_proto::record::Reference {
                                r#type: None,
                                record_identifier: Some(record_identifier_from_string(
                                    &container.user_id,
                                )),
                            }
                            .to_value()
