...
use crate::{
    ...
    util::{
        ...
        NSDictionaryClass, MessageTimings
    },
    OSConfig,
};
...
impl Balloon {
    ...
    fn unpack_raw(bp: &[u8]) -> Result<RawBalloonData, PushError> {
        let unpacked: NSDictionary<RawBalloonData> =
            plist::from_value(&KeyedArchive::expand(&ungzip(&bp)?)?)?;
        let NSDictionary {
            class: _,
            item: unpacked,
        } = unpacked;
...
impl ReactMessage {
    fn get_text(&self) -> String {
        self.custom_text
            .clone()
            .unwrap_or_else(|| self.reaction.get_text(&self.to_text))
    }
...
impl MMCSFile {
                ...
                get_message(
                    |loaded| {
                        ...
                        if c.as_unsigned_integer().unwrap() == 150
                            && i.as_unsigned_integer().unwrap() as u32 == msg_id
                        {
                            Some(loaded)
                        } else {
                            None
    ...
    pub async fn get_attachment(
        ...
        get_mmcs(
            &mmcs_config,
            authorized,
            vec![(self.signature.clone(), &self.object, recieve_container)],
            progress,
        )
        .await?;
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Message::Ack(ack) => {
                write!(
                    f,
                    "ack status={:?} error_status={:?} token={} uuid={} i={:?}",
                    ack.status,
                    ack.error_status,
                    ack.token.as_deref().unwrap_or("-"),
                    ack.uuid.as_deref().unwrap_or("-"),
                    ack.internal_id
                )
            }
...
pub struct MessageInst {
    ...
    pub timing_info: Option<MessageTimings>,
}

impl MessageInst {
    pub fn new(conversation: ConversationData, sender: &str, message: Message) -> MessageInst {
        Self::new_with_send_delivered(conversation, sender, message, None)
    }

    pub fn new_with_send_delivered(
        ...
        let send_delivered =
            send_delivered.unwrap_or_else(|| message.should_send_delivered(&conversation));
        MessageInst {
    ...
    pub fn prepare_send(&mut self, my_handles: &[String]) -> Vec<String> {
        ...
        let target_only_tokens = self
            .target
            .as_ref()
            .map(|targets| targets.iter().all(|t| matches!(t, MessageTarget::Token(_))))
            .unwrap_or(false);

        if !target_only_tokens
            && !conversation
                .participants
        ...
        let binary = match &self.message {
            ...
            Message::React(react) => {
                ...
                let raw = RawReactMessage {
                    ...
                    amrlc: 0,
                    amt: react.custom_amt.unwrap_or_else(|| react.reaction.get_cmd()),
            ...
            MessageType::IMessage => {
                ...
                let mut app_info: Option<Data> = None;
                    ...
                    if raw.xml.is_none() && normal.parts.is_multipart() {
            ...
            Message::Delivered => return Err(PushError::BadMsg),
            Message::Read => return Err(PushError::BadMsg),
            Message::MessageReadOnDevice => return Err(PushError::BadMsg),
            Message::PeerCacheInvalidate => return Err(PushError::BadMsg),
            Message::Error(_) => return Err(PushError::BadMsg),
            Message::Ack(_) => return Err(PushError::BadMsg),
            Message::NotifyAnyways => return Err(PushError::BadMsg),
            Message::Unschedule => return Err(PushError::BadMsg),
            Message::Unsend(msg) => {
    ...
    pub async fn from_raw(
        ...
        if let Ok(loaded) = plist::from_value::<RawReactMessage>(&value) {
            ...
            return wrapper.to_message(
                ...
                Message::React(ReactMessage {
                    ...
                    to_part,
                    to_text: "".to_string(),
                    custom_text: Some(loaded.text.clone()),
                    custom_amt: Some(loaded.amt),
                    reaction: msg,
        ...
        if let Ok(loaded) = plist::from_value::<RawIMessage>(&value) {
                ...
                (loaded.balloon_id.as_deref(), balloon_part)
            {
                match (|| {
                    ...
                    let payload: RichLink =
                        plist::from_value(&KeyedArchive::expand(unpacked.payload.as_ref())?)?;
                    debug!("c");
            ...
            debug!("e");
            debug!("loaded.participants: {:?}", loaded.participants);
            return wrapper.to_message(
                Some(ConversationData {
                    participants: loaded.participants.clone(),
                    cv_name: loaded.cv_name.clone(),
                ...
                Message::Message(NormalMessage {
                    ...
                    xml: loaded.xml.clone().or(loaded.live_xml.clone()),
