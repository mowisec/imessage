...
async fn get_find_my_headers<T: AnisetteProvider>(
    ...
    base_headers.insert(
        ...
        config.get_adi_mme_info("com.apple.AuthKit/1 (com.apple.findmy/375.20)"),
    );
...
impl<P: AnisetteProvider> FindMyClient<P> {
    pub async fn new(
        ...
        let client = FindMyFriendsClient::new(
            config.as_ref(),
            state.clone(),
            account,
            conn.clone(),
    ...
    pub async fn handle(&self, msg: APSMessage) -> Result<(), PushError> {
        if let Some(IDSRecvMessage {
            ...
            .receive_message(
                msg,
                &["com.apple.private.alloy.fmf", "com.apple.private.alloy.fmd"],
            )
            .await?
        {
    ...
    async fn make_request<T: for<'a> Deserialize<'a>>(
        ...
        let request = REQWEST
            .post(format!(
                ...
                self.server, self.state.dsid, path
            ))
            .headers(
                get_find_my_headers(
                    config,
    ...
    async fn make_request<T: for<'a> Deserialize<'a>>(
        ...
        let request = REQWEST
            .post(format!(
                ...
                self.server,
                if self.daemon {
                    format!("fmfd/{}", state.dsid)
                } else {
                    state.dsid.clone()
                },
                state.udid.to_uppercase(),
                path
            ))
            .headers(
                get_find_my_headers(
