...
use rustpush::{
    authenticate_apple,
    cloudkit::{
        record_identifier_from_string, CloudKitClient, CloudKitContainer, CloudKitOperation,
        CloudKitSession, CloudKitState,
    },
    facetime::{FTClient, FTMember, FTMessage, FTState, FACETIME_SERVICE, VIDEO_SERVICE},
    findmy::{FindMyClient, FindMyState, MULTIPLEX_SERVICE},
    get_gateways_for_mccmnc, login_apple_delegates,
    name_photo_sharing::{IMessageNameRecord, IMessageNicknameRecord, ProfilesClient},
    ...
    APSConnectionResource, APSState, Attachment, ConversationData, FileContainer, IDSNGMIdentity,
    IDSUser, IDSUserIdentity, IMClient, IndexedMessagePart, LoginDelegate, MMCSFile, MacOSConfig,
    Message, MessageInst, MessageParts, MessageType, NormalMessage, PushError, RelayConfig,
    ShareProfileMessage, SharedPoster, UpdateProfileMessage, MADRID_SERVICE,
};
...
async fn main() {
    ...
    let mut anisette_client = default_provider(
        config.get_gsa_config(&*connection.state.read().await),
    ...
    let mut users = if let Some(state) = saved_state.as_ref() {
        ...
        let path = PathBuf::from("statuskit.plist");
        std::fs::write(
            &path,
        ...
        let findmy = FindMyState::new(
            spd["DsPrsId"].as_unsigned_integer().unwrap().to_string(),
            spd["acname"].as_string().unwrap().to_string(),
            &mobileme,
        );
        ...
        let cloudkitstate = CloudKitState::new(
            spd["DsPrsId"].as_unsigned_integer().unwrap().to_string(),
            &mobileme,
        );
    ...
    *state.lock().unwrap() = Some(SavedState {
    ...
    let findmy_client = FindMyClient::new(
        ...
        Box::new(|state| {
            plist::to_file_xml(&PathBuf::from_str("findmy.plist").unwrap(), state)
    ...
    // Initialize StatusKit client (was missing, but used later).
    let statuskit_state: StatusKitState = plist::from_file("statuskit.plist").unwrap_or_default();
    let statuskit_client = StatusKitClient::new(
        connection.clone(),
        config.clone(),
        statuskit_state,
        anisette_client.clone(),
        client.identity.clone(),
        Box::new(|state| {
            plist::to_file_xml(&PathBuf::from_str("statuskit.plist").unwrap(), state)
                .expect("Failed to serialize plist!");
        }),
    )
    .await
    ...
    let shared_streams = SharedStreamClient::new(
        connection.clone(),
        config.clone(),
        state,
        anisette_client.clone(),
    ...
    let state: FTState =
        plist::from_file(&PathBuf::from_str("facetime.plist").unwrap()).unwrap_or_default();
    let facetime = FTClient::new(
        state,
        Box::new(|state| {
            plist::to_file_xml(&PathBuf::from_str("facetime.plist").unwrap(), state)
                .expect("Failed to serialize plist!");
        }),
        connection.clone(),
        client.identity.clone(),
    ...
    let manager = SyncController::new(
        shared_streams,
        PathBuf::from_str("syncstate.plist").unwrap(),
        FFMpegFilePackager::default(),
