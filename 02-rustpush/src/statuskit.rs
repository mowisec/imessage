...
use crate::{
    aps::{get_message, APSChannel, APSChannelIdentifier, APSInterestToken},
    ids::{
        identity_manager::{IDSSendMessage, Raw},
        user::QueryOptions,
        CompactECKey, IDSRecvMessage,
    },
    util::{base64_decode, base64_encode, decode_hex, encode_hex, plist_to_bin},
    APSConnection, APSMessage, IdentityManager, OSConfig, PushError,
};
use aes_gcm::KeyInit;
...
impl<T: AnisetteProvider + Send + Sync + 'static> StatusKitClient<T> {
    ...
    pub async fn ensure_channel(&self) -> Result<(), PushError> {
        ...
        let Some(token) = self
            ...
            .lock()
            .await
            .get_token("com.apple.gs.sharedchannels.auth")
            .await
        else {
    ...
    pub async fn update_channels(
        &self,
        new_channels: &mut HashMap<APSChannelIdentifier, u64>,
    ) -> Result<(), PushError> {
        new_channels.retain(|_k, v| *v > 0);

        let mut wanted_channels: HashSet<APSChannelIdentifier> =
            new_channels.keys().cloned().collect();
    ...
    pub async fn request_handles(
        self: &Arc<Self>,
        handles: &[String],
    ) -> (ChannelInterestToken<T>, Option<PushError>) {
    ...
    pub async fn request_channels(
        ...
        (
            ...
            self.update_channels(&mut *channel_lock).await.err(),
        )
    }
