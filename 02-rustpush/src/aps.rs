use std::{
    ...
    net::ToSocketAddrs,
    sync::{
        Arc, Weak, atomic::{AtomicU32, AtomicU64, Ordering}
    },
...
use crate::{
    activation::activate,
    auth::{do_signature, generate_nonce, NonceType},
    imessage::messages,
    ...
    util::{
        ...
        get_bag, KeyPair, Resource, ResourceManager, APNS_BAG,
    },
    OSConfig, PushError,
};
...
fn log_rtt_measurement(msg: &str) {
    error!("{msg}");
    ...
    fn to_raw(&self) -> APSRawMessage {
        match self {
            ...
            Self::SubscribeToChannels {
                ...
                let values = channels.iter().fold(
                    HashMap::<String, SubscribedTopic>::new(),
                    |mut acc, v| {
                        let entry = acc.entry(v.identifier.topic.clone()).or_insert_with(|| {
                            SubscribedTopic {
...
impl APSConnectionResource {
            ...
            loop {
                match ack_receiver.recv().await {
                    Ok(APSMessage::Notification {
                        ...
                        let _ = ack_ref
                            .send(APSMessage::Ack {
                                token: Some(ack_ref.get_token().await),
                                for_id: id,
                                status: 0,
                            })
                            .await;
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Ok(_) = keep_alive_ref.send(APSMessage::Ping).await {
                    let _ = keep_alive_ref
                        .wait_for_timeout(waiter, |msg| {
                            if let APSMessage::Pong = msg {

                                let end = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
                                let rtt_ms = (end - start).as_millis();
                                log_rtt_measurement(&format!("RTT to Server {} ms", rtt_ms));
    ...
    async fn update_topics(
        &self,
        topic_lock: &mut HashMap<&'static str, u64>,
    ) -> Result<(), PushError> {
        topic_lock.retain(|_k, v| *v > 0);
        self.filter(
            &topic_lock.keys().map(|k| *k).collect::<Vec<_>>(),
            &[],
            &[],
            &[],
        )
    ...
    pub async fn request_topics(
        &self,
        topics: Vec<&'static str>,
    ) -> (APSInterestToken, Option<PushError>) {
        ...
        (
            APSInterestToken {
                topics,
                aps: self.get_manager().await,
            },
            self.update_topics(&mut *topic_lock).await.err(),
        )
    }
    ...
    pub async fn send_message(
        ...
        .await?;
        let status = self
            .wait_for_timeout(self.subscribe().await, |msg| {
                let APSMessage::Ack {
                    token: _token,
                    for_id: _,
                    status,
