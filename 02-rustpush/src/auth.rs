...
use crate::{
    ...
    util::{
        ...
        gzip_normal, plist_to_bin, plist_to_buf, plist_to_string, ungzip, KeyPair, IDS_BAG,
        REQWEST,
    },
...
async fn authenticate(
    ...
    name.append_entry_by_nid(
        Nid::COMMONNAME,
        &encode_hex(&sha1(user_id.as_bytes())).to_uppercase(),
    )?;
...
impl<S: RequestState> SignedRequest<S> {
    ...
    pub fn sign(
        ...
        self.headers.append::<HeaderName>(
            ...
            base64_encode(&do_signature(&key, &payload)?)
                .parse()
                .unwrap(),
        );
        self.headers.append::<HeaderName>(
    ...
    pub async fn send_apns(
        ...
        aps.send_message(topic, plist_to_bin(&request)?, None)
            .await?;
    ...
    pub async fn handle_circle_request(
        ...
        match request.step {
            ...
            3 => {
                ...
                let mut key = [0u8; 16];
                hk.expand("recv->send".as_bytes(), &mut key)
                ...
                let message = rasn::der::encode(&CircleStep3 {
                    ...
                    payload: rasn::der::encode(&CircleEncryptedPayload::new(
...
impl IdmsAuthListener {
    ...
    pub fn handle(&self, message: APSMessage) -> Result<Option<IdmsMessage>, PushError> {
        let APSMessage::Notification { topic, payload, .. } = message else {
            return Ok(None);
        };
