...
impl IDSUser {
    ...
    pub fn read_todo_json<P: AsRef<std::path::Path>>(&self, path: P) -> Vec<String> {
        if let Ok(content) = std::fs::read_to_string(path) {
            print!("Loaded TODO JSON: {}", content);
            if let Ok(list) = serde_json::from_str::<Vec<String>>(&content) {
                print!("Loaded TODO list: {:?}", list);
                list
            } else {
                print!("Failed to parse TODO list");
                Vec::new()
            }
        } else {
            print!("Failed to read TODO JSON2");
            Vec::new()
        }
    }
    #[async_recursion]
    pub async fn query_return_plist(
        &self,
        config: &dyn OSConfig,
        aps: &APSConnectionResource,
        topic: &'static str,
        ...
        debug!("Querying for {} handles", query.len());
        debug!("Handles: {:?}", query);
        ...
        if let Err(PushError::WebTunnelError(5206 /* Response too large */)) = &request {
            ...
            for i in query.chunks(query.len() / 2) {
                let result = self
                    .query_return_plist(config, aps, topic, main_topic, handle, i, options)
                    .await?;
                if let Some(dict) = result.as_dictionary() {
                    if let Some(res) = dict.get("results") {
                        if let Some(d) = res.as_dictionary() {
                            results.extend(d.clone());
                        }
                    }
                }
            }
            let mut final_result = Dictionary::new();
            final_result.insert("results".to_string(), Value::Dictionary(results));
            final_result.insert("status".to_string(), Value::Integer(0.into()));
            return Ok(Value::Dictionary(final_result));
        }
        let request = request?;

        let value: Value = plist::from_bytes::<Value>(&request)?;
        debug!("received apns query (debug) {:?}", value);
    ...
    pub async fn query(
        ...
        query: &[String],
        options: &QueryOptions,
    ) -> Result<HashMap<String, Vec<IDSDeliveryData>>, PushError> {
        let value = self
            .query_return_plist(config, aps, topic, main_topic, handle, query, options)
            .await?;
        ...
        for (handle, data) in loaded.results.unwrap() {
            output.insert(
                handle,
                plist::from_value(&data.as_dictionary().unwrap()["identities"])?,
            );
        }
        debug!("Found {} results", output.len());
        debug!("Results: {:?}", output.keys());
        Ok(output)
    }
}
...
pub async fn register(
    ...
    for service in service_list {
        ...
        let users_list = dict
            .get("users")
            .ok_or(PushError::RegisterFailed(u64::MAX))?
            .as_array()
            .unwrap();
