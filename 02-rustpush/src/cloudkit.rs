...
use crate::{
    ...
    prepare_put,
    util::{decode_uleb128, encode_hex, encode_uleb128, gzip_normal, REQWEST},
    FileContainer, OSConfig, PushError,
};
...
fn undelimit_response(resp: &mut impl Read) -> Vec<Vec<u8>> {
    ...
    while let Ok(length) = decode_uleb128(resp) {
        ...
        resp.read_exact(&mut data)
            .expect("Failed to unlimit response");
        response.push(data);
    }
    response
}
...
impl<'t> CloudKitOperation<'t> {
    fn flow_control_key(&self) -> &'static str {
        match self {
            Self::UploadAsset { .. } | Self::SaveRecord { .. } | Self::DeleteRecord { .. } => {
                "CKDModifyRecordsOperation-Public"
            }
    ...
    fn record_delete_request(&self) -> Option<cloudkit_proto::RecordDeleteRequest> {
        let Self::DeleteRecord { record_id } = self else {
            return None;
        };
    ...
    fn get_retrieve_request(&self) -> Option<cloudkit_proto::RecordRetrieveRequest> {
        let Self::FetchRecord { record_id, assets } = self else {
            return None;
        };
    ...
    fn asset_upload_token_retrieve_request(
        &self,
    ) -> Option<cloudkit_proto::AssetUploadTokenRetrieveRequest> {
        let Self::UploadAsset {
            assets,
            mmcs_headers,
            mmcs_body,
        } = self
...
impl<'t> CloudKitContainer<'t> {
    ...
    pub async fn init<T: AnisetteProvider>(
        self,
        client: &CloudKitClient<T>,
    ) -> Result<CloudKitOpenContainer<'t>, PushError> {
        ...
        let response: CkInitResponse = self
            .headers(
                client,
...
impl<'t> CloudKitOpenContainer<'t> {
        ...
        let op = cloudkit_proto::RequestOperation {
            header: if is_first {
                Some(cloudkit_proto::request_operation::Header {
                    ...
                    device_identifier: if operation.is_fetch() {
                        None
                    } else {
                    ...
                    device_assigned_name: if operation.is_fetch() {
                        None
                    } else {
                        Some(config.get_device_name())
                    },
                    device_hardware_id: if operation.is_fetch() {
                        None
                    } else {
                    ...
                    isolation_level: Some(
                        cloudkit_proto::request_operation::header::IsolationLevel::Zone.into(),
                    ),
                    group: if let CloudKitOperation::FetchRecord { .. }
                    | CloudKitOperation::DeleteRecord { .. } = operation
                    {
                        None
                    } else {
                    ...
                    unk1: Some(0),
                    mmcs_headers: if let CloudKitOperation::FetchRecord { .. }
                    | CloudKitOperation::QueryRecord { .. } = operation
                    {
                    ...
                    tags: if let CloudKitOperation::DeleteRecord { .. }
                    | CloudKitOperation::QueryRecord { .. } = operation
                    {
                        vec![]
                    } else {
    ...
    pub async fn preform_operation<T: AnisetteProvider>(
        &self,
        session: &CloudKitSession,
        client: &CloudKitClient<T>,
        ops: &[CloudKitOperation<'_>],
    ) -> Result<Vec<ResponseOperation>, PushError> {
        let request = ops
            .iter()
            .enumerate()
            .map(|(idx, op)| self.build_request(&op, client.config.as_ref(), idx == 0))
            .collect::<Vec<_>>()
            .concat();
        ...
        let token: Vec<u8> = self
            .headers(
                client,
                REQWEST.post(ops.first().expect("No op?").link()),
                session,
            )
            .await?
        ...
        let response = undelimited
            .into_iter()
            .map(|u| Ok(ResponseOperation::decode(&mut Cursor::new(u))?))
    ...
    pub async fn get_record<T: AnisetteProvider>(
        &self,
        session: &CloudKitSession,
        client: &CloudKitClient<T>,
        assets: cloudkit_proto::AssetsToDownload,
        record_ids: &[String],
    ) -> Result<FetchedRecords, PushError> {
        let operation: Vec<CloudKitOperation<'_>> = record_ids
            .iter()
            .map(|r| CloudKitOperation::FetchRecord {
                record_id: r.clone(),
                assets: assets.clone(),
            })
            .collect();
    ...
    pub async fn query_record<T: AnisetteProvider, R: CloudKitRecord>(
        &self,
        session: &CloudKitSession,
        client: &CloudKitClient<T>,
        operation: CloudKitOperation<'_>,
    ) -> Result<(Vec<QueryResult<R>>, Vec<AssetGetResponse>), PushError> {
        let CloudKitOperation::QueryRecord { .. } = &operation else {
            panic!()
        }; // sanity check

        let response = self
            .preform_operation(&session, client, &[operation])
            .await?
            .remove(0);
    ...
    pub async fn get_assets<T: AnisetteProvider, V: Write + Send + Sync>(
        &self,
        client: &CloudKitClient<T>,
        ...
        let mmcs_config = MMCSConfig {
            mme_client_info: client.config.get_mme_clientinfo(
                "com.apple.cloudkit.CloudKitDaemon/1970 (com.apple.cloudd/1970)",
            ),
    ...
    pub async fn delete_record<T: AnisetteProvider>(
        &self,
        session: &CloudKitSession,
        client: &CloudKitClient<T>,
        record_id: &str,
    ) -> Result<(), PushError> {
        let operation: CloudKitOperation<'_> = CloudKitOperation::DeleteRecord {
            record_id: record_id.to_string(),
        };

        self.preform_operation(session, client, &[operation])
            .await?;

        Ok(())
    }

    pub async fn upload_asset<T: AnisetteProvider, F: Read + Send + Sync>(
        &self,
        session: &CloudKitSession,
        client: &CloudKitClient<T>,
        mut assets: Vec<CloudKitUploadRequest<F>>,
    ) -> Result<HashMap<String, cloudkit_proto::Asset>, PushError> {
        if assets.is_empty() {
        ...
        let asset_response = response
            .remove(0)
            .asset_upload_token_retrieve_response
            .expect("No asset response?");
